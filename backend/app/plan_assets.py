...
def validate_declared_type(detected: str | None, declared: str | None) -> tuple[str, str]:
    if detected not in ALLOWED_MEDIA_TYPES:
        raise AssetValidationError("Тип файла не разрешён или не распознан")
    normalized = (declared or "").split(";", 1)[0].strip().lower()
    if normalized not in DECLARED_ALIASES[detected]:
        raise AssetValidationError(
            f"MIME файла не совпадает с сигнатурой: заявлен {normalized or 'unknown'}, "
...
def validate_svg(data: bytes) -> None:
    if _SVG_BLOCKED.search(data):
        raise AssetValidationError("SVG содержит активный контент или внешнюю ссылку")
    try:
        root = ElementTree.fromstring(data)
    except ElementTree.ParseError as exc:
        raise AssetValidationError("Некорректный SVG") from exc
    if not root.tag.lower().endswith("svg"):
        raise AssetValidationError("Корневой элемент SVG не найден")
    if sum(1 for _ in root.iter()) > 50_000:
        raise AssetValidationError("SVG содержит слишком много элементов")
